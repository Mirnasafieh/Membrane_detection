...
def barplot_E3E4(data,parameter):
    graph=sns.barplot(x="cell genotype", y=parameter, palette="vlag",  data=data)
    return graph
...
def all_compartments_bars(data):
    g = sns.FacetGrid(df1, col="compartment", height=4, aspect=.5)
    result= g.map(sns.barplot, "cell genotype", "M1", palette='vlag')
    return result
...
if __name__ == "__main__":
    ...
    a=barplot_E3E4(df1,"IOD")
    b=all_compartments_lines(df1)
    c=all_compartments_bars(df1)
    plt.show()
...
'prism', 'prism_r', 'rainbow', 'rainbow_r', 'rocket', 'rocket_r', 'seismic', 'seismic_r', 'spring', 'spring_r', 'summer', 'summer_r', 'tab10', 'tab10_r', 'tab20', 'tab20_r', 
